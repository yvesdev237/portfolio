...
const About = () => {
  const infos = [
    ...
    { label: 'Availability', text: <span className="flex items-center gap-2 text-emerald-300"><FaDotCircle /> Open for work</span> },
    ...
    >
      <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
        <div className="space-y-4">
          <div className="inline-flex rounded-full border border-cyan-400/30 bg-cyan-400/10 px-3 py-1 text-sm font-medium text-cyan-200">About me</div>
          <img src={profile} alt="Yves profile" className="h-72 w-full rounded-[24px] object-cover shadow-[0_12px_40px_rgba(2,12,27,0.34)]" />
        </div>

        <div className="space-y-5">
          <h2 className="text-3xl font-semibold text-white sm:text-4xl">
            Passionate about building <span className="accent-gradient">great software</span>
          </h2>
          ...
          <div className="rounded-[24px] border border-white/10 bg-white/5 p-4">
            <h3 className="text-sm font-semibold uppercase tracking-[0.25em] text-slate-400">What I focus on</h3>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {infos.map((info, index) => (
              ...
              >
                <p className="text-sm uppercase tracking-[0.2em] text-slate-400">{info.label}</p>
