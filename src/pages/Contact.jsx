      ...
      <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          ...
          <div className="flex flex-wrap gap-3">
            {social.map((item) => (
              <a
                ...
                className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm font-medium text-slate-200 transition hover:border-cyan-300/40 hover:bg-cyan-400/10"
              >
                {item.icon} {item.name}
              </a>
            ))}
        ...
        >
          ...
          <button
            type="submit"
            disabled={loading}
            className="mt-5 inline-flex items-center justify-center gap-2 rounded-full bg-gradient-to-r from-cyan-400 to-blue-600 px-5 py-3 text-sm font-semibold text-slate-950 transition hover:scale-[1.01]"
          >
            {loading ? (
